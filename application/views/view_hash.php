...
<table>
	...
	?>
	<tr <?= $alt ? 'class="alt"' : ''?>>
		<td><div><?= format_html($hkey)?></div></td>
		<td><div><?= nl2br(format_html($value))?></div></td>
		<td>
			<div> 
				<a href="<?= manager_site_url('edit', 'index', 'key=' . urlencode($key) . '&type=' . $type . '&hkey=' .  urlencode($hkey)); ?>">
		...
		<td>
			<div> 
				<a href="<?= manager_site_url('delete', 'index', 'key=' . urlencode($key) . '&type=' . $type . '&hkey=' .  urlencode($hkey)); ?>" class="delval">
